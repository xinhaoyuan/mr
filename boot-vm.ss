...
(define (vm-prog:entry-regs-size prog)
...
(define vm-instruction-head:if             'if)
(define vm-instruction-head:goto           'goto)
          ...
          (let ((instruction (vm-trace:instruction trace pc)))
            ...
            (let ((head (vm-instruction:arg instruction 0)))
              (cond
               ...
               ((eq? head vm-instruction-head:if)
               ...
               ((eq? head vm-instruction-head:goto)
...
(define (vm:execute-prog prog)
  (let ((regs (make-vector (vm-prog:entry-regs-size prog) '())))
    (vm:execute prog
